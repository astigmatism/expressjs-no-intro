...
const screensRoot = path.join(mediaRoot, 'screens');

module.exports = new (function() {

    var _self = this;

    this.Audit = function(system, screenType, callback) {

        var screensPath = path.join(screensRoot, screenType, system);
        var files;

        //get dir listing
        try {
            files = fs.readdirSync(screensPath);
        }
        catch (e) {
        ...
        MasterFiles.Get(system, 'roms', (data) => {
            MasterFiles.Get(system, 'screens-' + screenType, (masterfile, masterfileFileName) => {
                ...
                for (var title in data.data) {
                    ...
                    //dont offer the file for play, so dont consider it when making matches
                    if (data.data[title] == '') {
                        continue;
                    }
                    titles.push(title);
    ...
    this.GetAuditPreview = function(system, screenType, filename, callback) {

        var screensPath = path.join(screensRoot, screenType, system, filename);

        fs.readFile(screensPath, (err, buffer) => {
            if (err) return callback(err)
